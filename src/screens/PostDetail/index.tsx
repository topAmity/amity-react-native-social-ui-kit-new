...
import { type IMentionPosition } from '../CreatePost';
...
const PostDetail = () => {
  ...
  const [commentCollection, setCommentCollection] =
    useState<Amity.LiveCollection<Amity.InternalComment>>();
  ...
  function getCommentsByPostId(postId: string) {
    CommentRepository.getComments(
      ...
      (data: Amity.LiveCollection<Amity.InternalComment>) => {
  ...
  const onSelectUserMention = (user: ISearchItem) => {
    ...
    const newTextAfterReplacement =
      inputMessage.slice(0, cursorIndex - (currentSearchUserName as string).length) +
      user.displayName +
    ...
    const position: IMentionPosition = {
      ...
      index: cursorIndex - 1 - (currentSearchUserName as string).length,
  ...
  return loading ? (
    ...
    >
      ...
      {isShowMention && (
        <MentionPopup
          userName={currentSearchUserName as string}
